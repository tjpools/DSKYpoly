...
// Simple quadratic solver implementation
void solve_poly_2(double a, double b, double c,
    ...
    if (discriminant >= 0) {
        ...
        *r1_real = (-b + sqrt_d) / (2 * a);
        *r1_imag = 0.0;
        *r2_real = (-b - sqrt_d) / (2 * a);
        *r2_imag = 0.0;
    } else {
        // Complex roots
        double sqrt_neg_d = sqrt(-discriminant);
        *r1_real = -b / (2 * a);
        *r1_imag = sqrt_neg_d / (2 * a);
        *r2_real = -b / (2 * a);
        *r2_imag = -sqrt_neg_d / (2 * a);
    }
}
